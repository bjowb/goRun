import { getUserProfile, getMealSuggestions, getFoodLogStats } from "@/actions/dashboard";
...
import { ArrowUpCircle, Award, Calendar, ChevronRight, Clipboard, Clock, CookingPot, InfoIcon, Leaf, Library, LineChart, ListTodo, PlusCircle, Utensils, Zap } from "lucide-react";
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center">
      <div className="flex flex-col items-center gap-2">
        ...
        <h3 className="text-xl font-semibold">Loading your nutrition dashboard...</h3>
      </div>
    </div>
  );
}
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Let's plan your healthy meals for today</p>
        </div>
      </div>
      ...
      <CardContent>
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-medium">{calories} / {calorieGoal} kcal</span>
          </div>
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-sm">{protein.toFixed(1)}g ({proteinPercentage}%)</span>
            </div>
            <Progress value={proteinPercentage} className="h-1.5 bg-blue-100" indicatorClassName="bg-blue-500" />
          </div>
          
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm">Carbs</span>
              <span className="text-sm">{carbs.toFixed(1)}g ({carbsPercentage}%)</span>
            </div>
            <Progress value={carbsPercentage} className="h-1.5 bg-green-100" indicatorClassName="bg-green-500" />
          </div>
          
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm">Fat</span>
              <span className="text-sm">{fat.toFixed(1)}g ({fatPercentage}%)</span>
            </div>
            <Progress value={fatPercentage} className="h-1.5 bg-yellow-100" indicatorClassName="bg-yellow-500" />
          </div>
        </div>
      </CardContent>
...
function MealPlan({ mealSuggestion }) {
    ...
    return (
      <Card className="col-span-12 md:col-span-8">
        <CardHeader>
          ...
          <CardDescription>Complete your profile to get meal suggestions</CardDescription>
        </CardHeader>
        <CardContent className="flex items-center justify-center py-10">
          <div className="text-center">
            ...
            <p className="text-muted-foreground max-w-md">
              Update your profile with your dietary preferences and physical details to get personalized meal suggestions.
            </p>
  ...
  const sortedMeals = [...mealSuggestion.meals].sort((a, b) => {
    return mealTypeOrder[a.mealType.toLowerCase()] - mealTypeOrder[b.mealType.toLowerCase()];
  });
            ...
            <div key={index} className="rounded-lg border bg-card p-4">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2">
                  <Badge variant={
                    meal.mealType.toLowerCase() === 'breakfast' ? 'default' : 
                    meal.mealType.toLowerCase() === 'lunch' ? 'secondary' :
                    meal.mealType.toLowerCase() === 'dinner' ? 'destructive' : 'outline'
...
function UserStats({ user }) {
  ...
  if (user.height && user.weight) {
    ...
    if (bmi < 18.5) bmiCategory = { label: "Underweight", color: "text-blue-500" };
    else if (bmi < 25) bmiCategory = { label: "Normal", color: "text-green-500" };
    else if (bmi < 30) bmiCategory = { label: "Overweight", color: "text-yellow-500" };
      ...
      <CardContent className="space-y-4">
        ...
        {bmi && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-center text-muted-foreground">{bmi.toFixed(1)}</p>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm">
            ...
            <span className="font-medium">{user.dietaryPreference || "Not specified"}</span>
          </p>
        </div>
          ...
          {foodLogs.slice(0, 5).map((log, index) => (
            <div key={index} className="flex items-center justify-between p-2 rounded-md hover:bg-muted">
              ...
              <div className="text-right">
                <p className="font-medium">{log.foodItem.calories * log.quantity} kcal</p>
                <p className="text-xs text-muted-foreground">{log.quantity} serving(s)</p>
              </div>
            </div>
          ))}
        </div>
...
function ProgressChart({ progressRecords }) {
  ...
  return (
    <Card className="col-span-12 md:col-span-6">
            ...
            {progressRecords.map((record, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {new Date(record.loggedAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                  </div>
                </div>
              );
            })}
      ...
      <CardFooter className="border-t pt-4">
        <Button variant="outline" size="sm" className="w-full">Log New Weight</Button>
      </CardFooter>
    </Card>
  );
}
...
function QuickActions() {
  const actions = [
    { icon: <PlusCircle className="h-5 w-5" />, label: "Log Food", color: "bg-blue-100 text-blue-600" },
    { icon: <Utensils className="h-5 w-5" />, label: "Meal Plan", color: "bg-green-100 text-green-600" },
    { icon: <Zap className="h-5 w-5" />, label: "Track Weight", color: "bg-yellow-100 text-yellow-600" },
    { icon: <Library className="h-5 w-5" />, label: "Recipes", color: "bg-purple-100 text-purple-600" },
  ];
      ...
      <Tabs defaultValue="overview" className="mt-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <NutritionSummary stats={foodStats || { calories: 0, protein: 0, carbs: 0, fat: 0 }} />
        ...
        <TabsContent value="meals">
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground">Your personalized meal plans and recipes will appear here.</p>
            
            <Card>
              <CardHeader>
                <CardTitle>Generate a Custom Meal Plan</CardTitle>
                <CardDescription>Get personalized meal suggestions based on your preferences</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-center py-10">
                  <div className="text-center">
        ...
        <TabsContent value="progress">
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground">Track your fitness and nutrition journey over time.</p>
            
            <Card>
              <CardHeader>
                <CardTitle>Progress Tracking</CardTitle>
                <CardDescription>Your weight, measurements, and other metrics over time</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-center py-10">
                  <div className="text-center">
